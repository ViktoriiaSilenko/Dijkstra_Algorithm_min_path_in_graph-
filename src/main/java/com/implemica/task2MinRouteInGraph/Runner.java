...
public class Runner {
	...
	private static void verticesHandling (BufferedReader br, int attemptsNumber) {
					...
					for (int vertexIndex = 0; vertexIndex < vertexNumber; vertexIndex++) {
						...
						if (vertexNeighboursNumber > 0 && vertexNeighboursNumber <= vertexNumber - 1) {
							...
							for (int vertexNeighboursIndex = 0; vertexNeighboursIndex < vertexNeighboursNumber; vertexNeighboursIndex++) {
								System.out.println(String.format(MessageConstants.MSG_NR_COST_INPUT, cityName, vertexNumber));
		...
		do {
			...
			try {
				...
				if (attemptsNumber > 0 && attemptsNumber <= MaxNumberConstants.MAX_TEST_NUMBER) {
					...
					verticesHandling(br, attemptsNumber);
